...
**Withdrawal — two directions, both named acts.** The capability can be taken away by the **admin** who gates it (the app-domain `revoke_capture`: admin-scoped, targets one member, and turns an existing grant off), and it can be given up by **its own subject** (the app-domain `relinquish_capture`). Neither is the same act as `decline_capture`, which turns down an outstanding *request* and never touches a grant already in force, and neither removes the member from the Click — departure is [section 6.4](#64-biometric-group-key-rotation-on-member-removal)'s separate act with its own rotation consequences.
