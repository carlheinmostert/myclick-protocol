...
- **Embedding** — the ciphertext. A roughly 2048-byte face vector extracted on-device, persisted only in encrypted form. It is the only biometric used for matching — each owned capture may additionally carry an encrypted crop sidecar (section 4.8), an image rather than a vector, stored as ciphertext neither the server nor anyone but the owner can decrypt — and it is not reversible into a photograph.
...
**This revises a claim.** Earlier versions of this protocol stated that embeddings are the only persisted biometric artifact, and that the sidecar never leaves the device. Neither is the whole truth any longer, and we say so plainly: with sidecars, each owned enrolment capture may persist **one encrypted face crop** — held on the owner's device and, sealed as ciphertext the server cannot read, mirrored to our server so it survives reinstalling the app or moving to a new phone. The embedding remains the only biometric used for *matching*. Both embeddings and sidecars leave the device only as ciphertext that no one but the owner can decrypt. The sidecar is a deliberate, bounded exception — an image, not a vector — and every rule below exists to keep it bounded.
...
**Encryption and at-rest posture.** Each sidecar is encrypted with AES-GCM under its own per-file content key. That per-file key is **wrapped under the person's content key** — the same vault-recoverable content key that locks the person's embeddings and name ([section 6.3](#63-content-key-indirection)) — *not* under the device store key that protects on-disk photo blobs ([section 8.8](#88-the-on-device-encrypted-store-lanes-keys-and-the-journal)). This is the one change that lets a sidecar be recovered: because the person's content key is itself wrapped under the owner's escrowed vault key ([section 10](#10-key-escrow-and-recovery)), anything that wrap protects comes back on a new device. The local copy is written `NSFileProtectionComplete` and excluded from all backups, exactly as before; what changes is that the local copy is now a **cache**, not the only copy.

**Server-stored, E2EE, and recoverable.** The sidecar ciphertext and its wrapped per-file key are also stored on our server as **opaque blobs**, keyed to the capture. The server holds them exactly as it holds encrypted embeddings and wrapped content keys ([section 5.1](#51-what-the-server-holds)): it cannot decrypt them, because it never holds the person's content key. On a fresh install or a new phone, the device recovers the person's content key through the ordinary escrow path, fetches each sidecar's ciphertext and wrap from the server, and rebuilds the local cache — so a parent's roster of faces survives reinstalling the app and migrating devices, the same way embeddings and names already do. This closes the one gap where a delete-and-reinstall used to lose the owner's enrolment imagery. It does **not** widen the consent boundary: sidecars still exist only for owned biometrics, and the server still never sees a decryptable face — only ciphertext, as with every other artifact it stores ([section 5.2](#52-what-the-server-never-holds)). This is the same posture the obscured-photo relay already takes ([section 11](#11-streaming-relay-and-distribution)): encrypted imagery the server routes but cannot read.
...
**Lifecycle — 1:1 with its capture, no orphans.** A sidecar is crypto-shredded the moment its capture is pruned, evicted, retaken, or its person deleted: its per-file key is destroyed (its wrap removed) and the ciphertext is deleted **from both the local cache and the server**, so neither copy outlives the capture. One capture, at most one sidecar, with exactly the capture's lifetime. On every launch the app reconciles sidecars against captures and shreds any orphan — the same journal pattern as the capture scratch posture ([section 8.4](#84-the-hardened-flow-b)); reconciliation now also reaps server blobs whose capture is gone. Deleting a person destroys that person's content key, which crypto-shreds every artifact sealed under it — embeddings, name, and sidecars alike — in one stroke.
...
- **Encrypted enrolment crop sidecars.** At most one encrypted face crop per owned enrolment capture, as ciphertext, each with its per-file key wrapped under the person's content key ([section 4.8](#48-enrolment-capture-sidecars-stored-face-crops)). Stored so a parent's enrolment imagery survives reinstall and device migration. The server cannot decrypt these, because it never holds the content key — exactly as with embeddings.
...
- **Any *decryptable* photo or face image.** No image the server can read is ever uploaded — not at enrolment, not at capture, not at import. Enrolment-capture sidecars ([section 4.8](#48-enrolment-capture-sidecars-stored-face-crops)) are now mirrored to the server, but only as ciphertext whose key the server never holds; the obscured-photo relay ([section 11](#11-streaming-relay-and-distribution)) takes the same posture. The server stores encrypted bytes it cannot turn back into a face: it has never been able to see a child's face and never will be.
...
Recovery is by **iCloud Keychain escrow** (the decision is recorded in [section 3.2](#32-two-decisions-locked-here)), and it escrows two things: the **account vault key** and the **material needed to rejoin the account's Clicks**. The second part matters as much as the first — account recovery restores full Click participation, not just the private vault. A recovered parent gets back their own embeddings, the names they gave each person, and their enrolment crop sidecars ([section 4.8](#48-enrolment-capture-sidecars-stored-face-crops)) — everything sealed under their content keys — and is able to take part in the school Click, the class Click, and the family again, rather than being left holding their own data but locked out of everyone else's. The sidecars come back because they, like embeddings, are stored as ciphertext under content keys the recovered vault key unwraps; nothing of the owner's enrolment is left behind on the old device.
...
All six are deliberate v1 concessions of exactly the kind [section 1.2](#12-what-we-concede-accepted-risks-stated-plainly) catalogues, recorded for the DPIA, and folded (where applicable) into the v2 metadata-hiding scope deferred in [section 12](#12-open-questions).
