...
2. [Canonical model](#2-canonical-model)
   - [2.1 The world in practice](#21-the-world-in-practice)
...
6. [Per-Click group key (the ratchet)](#6-per-click-group-key-the-ratchet)
   ...
   - [6.2.3 Exact biometric group-key recipient wrap](#623-exact-biometric-group-key-recipient-wrap)
   - [6.9 Exact symmetric key-wrap records](#69-exact-symmetric-key-wrap-records)
...
- **World** — the cryptographic security domain: the scope inside which every other identifier in this document is unique and authoritative, and outside which none of them means anything. It is named by `world_id`, a stable CSPRNG UUIDv4 assigned once to a deployment — production, staging, a local development stack, or an independent fork — and immutable for that deployment's entire life. Every context binding in this protocol carries it as the **outermost** field, so a key, row, publication, or wrap minted in one world cannot authenticate in another **even when the ids inside it collide**. That collision case is the reason the field exists rather than an unlucky edge: a cloned, dumped, or restored database makes colliding ids the expected outcome, not a remote one. One deployment is exactly one world; a world is never subdivided into sub-worlds and never rotated in place ([section 2.1](#21-the-world-in-practice)).
  *In plain terms: which installation of myClick this is — the outermost fence, so a key from the test system can never open anything on the real one.*
...
- **ContentKeyWrap** — a person's content key encrypted under one outer key: either the owner's vault key (owner-private access) or a specific Click GroupKeyVersion (shared recognition within that Click). The embedding ciphertext is stored once; only this small wrap is multiplied per audience. Both arms have pinned, context-bound wire formats — `content-key-vault-wrap-v1` and `content-key-group-wrap-v1` — so a wrap cannot be moved to another person, Click, or key version without detection ([section 6.9](#69-exact-symmetric-key-wrap-records)).
  *In plain terms: one person's content key, locked for one audience — and only that person, only that audience.*
- **GroupKeyWrap** — a GroupKeyVersion encrypted under one member's identity public key, so only that member's Secure Enclave can unwrap it. This is how a group key reaches each member without the server ever holding a usable copy. Its wire format is pinned as `group-key-recipient-wrap-v1` and context-bound, so it cannot be moved to another Click, key version, or device without detection ([section 6.2.3](#623-exact-biometric-group-key-recipient-wrap)).
  *In plain terms: the group key, locked so only one member can open it — and only for that one Click, that one key generation, and that one device.*
...
### 2.1 The world in practice

`world_id` is the first field of every context binding this protocol defines — the device-key proof and ledger envelope of [section 6.2](#62-distribution-rides-on-admin-approval), the Stream-epoch and biometric group-key recipient wraps of [sections 6.2.2](#622-separate-stream-history-epochs) and [6.2.3](#623-exact-biometric-group-key-recipient-wrap), all three symmetric wrap records of [section 6.9](#69-exact-symmetric-key-wrap-records), and the Stream envelope salts and `AAD_base` of [sections 11.2](#112-per-blob-content-key-wrapped-under-the-stream-epoch-key) and [11.2.1](#1121-versioned-authenticated-envelope-and-aad). Nothing in the protocol derives a key without it. That makes it the one construct whose operational questions must be answered before any of the others can be implemented, and this section answers them.

**What makes two deployments different worlds.** The test is not whether two deployments are logically related, administratively connected, or running the same build. It is exactly one question: **may a ciphertext minted there authenticate here?** If the answer is no, they are different worlds. Two deployments are the *same* world only when they share one authoritative id space *and* one body of key material — the same database, or a faithful replica of it serving the same clients. Production and staging are different worlds. A local development stack is a different world from both. An independent fork of the open-source server is a different world from every myClick deployment. The sharpest case, and the one the binding is really for, is a staging stack restored from a production dump: it is emphatically a different world *because* every id inside it collides with a real one by construction, so id equality proves nothing and only the world field separates them.

A world is **not** a tenancy or customer boundary. Two schools, two families, or two countries served by one deployment are Clicks within one world, not separate worlds. The word "domain" in "security domain" invites that misreading, so it is ruled out here explicitly: the world fences *installations of the system* apart, never *users of one installation*. Users are fenced apart by Click, group key, and membership, which is what the rest of this document is about.

**Who mints it, when, and how clients learn it.** The relay mints `world_id` **once, at deployment bootstrap, before the first account, Click, person, or key exists in that deployment**, as a CSPRNG UUIDv4 with the mandatory version and variant bits of [section 11.2.1](#1121-versioned-authenticated-envelope-and-aad). A client MUST NOT mint it: there are many clients per deployment and they would disagree, and disagreement here means every derived wrapping key differs and nothing opens. Minting is therefore server-side by necessity, not by preference.

The value is **public and requires integrity and consistency, not confidentiality** — it is bound into AAD in the clear, appears in relay rows, and is visible in the metadata inventory of [section 11.18](#1118-metadata-leaks-and-conceded-trust-boundaries). A client obtains it over the authenticated channel and **pins it locally on first successful bootstrap**, and from that point treats it as immutable local state rather than as a field to be re-read and re-adopted on each call.

**The honest limit of that.** The relay serves `world_id`, and the relay is precisely the party every one of these bindings exists to distrust. So state the property accurately: the world binding is a **separation** mechanism, not an **authentication** one. It guarantees that artifacts belonging to different worlds cannot be interchanged; it does not, by itself, prove to a client which world it is in. Pinning on first contact is what bounds the relay's remaining power — a relay that lies about `world_id` at bootstrap gives the client a world in which nothing it later receives will authenticate, and a relay that changes its answer afterwards is refused rather than followed. The relay can therefore deny service; it cannot silently migrate a client from one world into another, which is the property that matters, because a silent migration is what would let a foreign ciphertext land in a slot the client believes is its own. Strengthening this into genuine authentication — binding `world_id` into the out-of-band invite or device-link secret of [section 6.2](#62-distribution-rides-on-admin-approval), so a joining device learns the world from a human-carried channel rather than from the relay — is a sound extension, and it is deliberately **not** required here; it is recorded as available work rather than specified, because the pin already closes the migration case and the invite construction has its own review to pass.

**A client that does not know its world, or knows a stale one.** Because `world_id` is the first field of every context, a client without one can encode no context, derive no wrapping key, and perform **no protocol cryptography whatsoever** — not a degraded subset, none. There is accordingly no default, no fallback constant, no nil UUID, and no all-zero sentinel; [section 11.2.1](#1121-versioned-authenticated-envelope-and-aad) forbids nil for UUID fields unless a field explicitly permits it, and this field does not permit it. A client that cannot determine its world MUST fail loud and block every enrolment, distribution, roster decrypt, publish, and fetch path until it can. Guessing a world is worse than stopping, because a guessed world produces artifacts that are individually well-formed and permanently unopenable.

A *stale* world — a pinned value that no longer matches the deployment — has exactly three causes, and a client cannot distinguish them: the deployment was destructively rebootstrapped into a fresh world, the client is pointed at a different deployment than the one it bootstrapped against, or the relay is lying. All three take the same response, which is why the indistinguishability is tolerable. The client MUST stop, MUST treat every locally held world-bound artifact — cached keys, wraps, roster material, display caches, outbound records — as unreadable rather than as material to be re-interpreted, and MUST recover only by destructive local reset and fresh bootstrap. It MUST NOT silently re-pin: re-pinning on a changed answer is exactly the relay-driven world migration the pin exists to prevent, and it would convert a loud failure into a silent adoption of someone else's world.

**One world per deployment, and the shape that follows.** `world_id` is deployment-wide. One deployment hosts exactly one world, and the protocol offers no construction for a second: there is no world hierarchy, no world parameter on any RPC, and no operation that moves an artifact between worlds. This is not merely the current simplification — [section 11.2.1](#1121-versioned-authenticated-envelope-and-aad) already requires that choosing a fresh world destroys **all** world-bound rows, keys, wraps, and caches before any subsystem reopens, and that requirement only makes sense if a deployment has one world to replace rather than a set to add to. Rotation in place does not exist, for the same reason: every wrapping key in the deployment is derived under the old world, so a rotation and a total destructive rebootstrap are the same event described two ways.

The storage shape follows from that, and two plausible shapes are wrong:

- It is **not a column on each table.** A per-row `world_id` models a database whose rows may belong to different worlds, which is precisely what this section forbids, and it manufactures a wrong-world-row failure mode that cannot exist if the value has nowhere to vary.
- It is **not a mutable configuration value** — an environment variable, a deployment setting, a connection parameter. The value must be durable, auditable, and provably assigned exactly once, and an environment variable is silently changeable by anyone who can restart a process. A value whose silent change destroys every key in the deployment must not live somewhere silent change is routine.

The conforming shape is a **deployment-level singleton**: one durably stored record, its single-record-ness enforced by the storage layer rather than by convention, immutable after assignment, and readable through the deployment's ordinary authenticated read surface. Other tables do not carry the world; their rows are implicitly of the one world their deployment has.

That is distinct from — and must not be confused with — the requirement that **stored wrap records** represent their full context tuple explicitly ([sections 6.2.3](#623-exact-biometric-group-key-recipient-wrap) and [6.9](#69-exact-symmetric-key-wrap-records)). Those records carry a `world_id` field, and they must. The two are different objects doing different jobs: the singleton is the **client's derivation source**, the value it reconstructs context from; the field inside a record is the **relay's routing copy**, a locator that the holder byte-compares against its own pinned value and never treats as a trust root. A holder that derived its context from the record's own copy would be authenticating a record against itself, which is no check at all. Where an implementation decomposes a stored wrap into columns rather than keeping it as an opaque record, the `world_id` field travels with the rest of that tuple; it does not become a partition key.
...
- **AEAD mode — AES-256-GCM.** Every symmetric encryption in the biometric hierarchy — the vault key and each group key protecting content keys, and each per-person content key protecting its embedding template set — uses **AES-256-GCM**. This is the same authenticated mode sections 4.8 / 8.8 fix for the on-device store; section 3 now states it explicitly for the escrowed hierarchy too. **Where a symmetric wrap crosses the relay it is not a bare seal under the outer key.** The group key escrowed under a vault key and each per-person content key wrapped under a vault key or a GroupKeyVersion are sealed under an HKDF-derived per-slot wrapping key with their context bound into both the salt and the GCM AAD, in the formats pinned at [section 6.9](#69-exact-symmetric-key-wrap-records); the outer key alone would say who may open a wrap without ever saying which wrap it is. The remaining symmetric encryption that never leaves the device — the on-disk roster cache of [section 7.4](#74-roster-key-wrapping--secure-enclave-self-ecdh) — is unaffected.
- **Asymmetric wrap — ECIES over P-256.** Wrapping a symmetric key *under an identity public key* — the vault key on-device at rest, and each group key for delivery to a member (section 3.1) — uses **ECIES over P-256**: an ephemeral ECDH to the recipient's P-256 public key, HKDF-SHA256 to derive the wrapping key, then AES-256-GCM. **The two uses are now pinned differently, because only one of them crosses the relay.** A group key *delivered to a member device* has its wire format pinned as `group-key-recipient-wrap-v1` in [section 6.2.3](#623-exact-biometric-group-key-recipient-wrap), with a mandatory context binding over `(world_id, click_id, group_key_version_id, recipient_account_id, recipient_device_id)` in both the HKDF salt and the GCM AAD; an earlier draft left this delivery format implementation-defined and opaque, and that is superseded — a wrap that travels through the relay must not be relocatable between Clicks, key versions, or devices. The *on-device-at-rest* vault-key wrap never leaves the phone, so it remains self-describing local storage and crosses the Storage Port only as an opaque blob.
...
#### 6.2.3 Exact biometric group-key recipient wrap

The group-key delivery of [section 6.2](#62-distribution-rides-on-admin-approval) is a wire artifact, and it is pinned here exactly as [section 6.2.2](#622-separate-stream-history-epochs) pins the Stream-epoch recipient wrap. It is not implementation-defined "opaque ECIES"; this supersedes that phrasing in [section 3.3](#33-concrete-primitives-locked-2026-06-18). For each recipient device, the wrapping device first authenticates §6.2's Click device-key ledger and selects the exact pinned public key for the target `(recipient_account_id, recipient_device_id)`; a raw relay-returned key column is never accepted. It then generates a fresh ephemeral P-256 keypair — one per recipient device, never reused across recipients or across GroupKeyVersions — and performs ECDH with that recipient identity public key. Both public keys use the 65-byte SEC 1/X9.63 uncompressed representation (`0x04 || X[32] || Y[32]`), and invalid/off-curve/non-canonical points are rejected. Using the canonical `ENCODE` of [section 11.2.1](#1121-versioned-authenticated-envelope-and-aad):

```text
group_key_delivery_context =
  ENCODE("myclick.group-key-delivery.v1",
         world_id, click_id, group_key_version_id,
         recipient_account_id, recipient_device_id)

K_group_delivery = HKDF-SHA256(
  IKM = P256-ECDH(ephemeral_private, recipient_identity_public),
  salt = SHA-256(group_key_delivery_context),
  info = "myclick.group-key-delivery-key.v1",
  L = 32)
```

AES-256-GCM then seals the exact 32-byte biometric group key under `K_group_delivery`, with one fresh 96-bit CSPRNG nonce and `group_key_delivery_context` as AAD. The wire record is the closed `group-key-recipient-wrap-v1` map in [section 11.2.2](#1122-canonical-v1-records): version, context ids, recipient ids, ephemeral public key, nonce, 32-byte ciphertext, and 16-byte tag. Each derived key performs exactly one successful seal; a rejected submission is retried byte-for-byte or abandoned, never resealed under a second nonce. Admission ([section 6.2](#62-distribution-rides-on-admin-approval)) and every removal rotation ([section 6.4](#64-biometric-group-key-rotation-on-member-removal)) mint a fresh wrap per eligible device under the then-current `group_key_version_id`. The data model and RPC implementation MUST represent `(world_id, click_id, group_key_version_id, recipient_account_id, recipient_device_id)` explicitly for every stored wrap; inferring which key a wrap carries from current membership, or from the row it happens to sit in, is non-conforming.

**The recipient rebuilds the context from the slot it is filling, never from the record.** The five context ids carried inside the record are the relay's routing copy — locators, never a trust root, exactly as the raw key columns of §6.2 are. Before opening, a recipient reconstructs `group_key_delivery_context` from the world and Click it is installing into, the `group_key_version_id` that slot claims, and its own account and device ids, and byte-compares the record's copies against that reconstruction. Only then does it open. Because §6.2's ledger envelope is itself sealed under a key derived from `K_group` and a `ledger_context` carrying the same `(world_id, click_id, group_key_version_id)`, the ordering §6.2 already requires is unchanged and now load-bearing twice: the recipient opens the wrap, then authenticates the ledger with the recovered key, finds its own exact `(account, device, public key, fingerprint)` entry, and only then retains the key. A wrap presented in the wrong slot fails the GCM tag at the first step; if it somehow did not, the ledger would fail to open at the second. Unknown version, invalid point, context mismatch, duplicate recipient wrap, failed tag, or a ledger without the recipient's exact entry rejects the grant and retains nothing.

**Why the binding exists.** Without it, a wrap says only "these 32 bytes are sealed to your identity key" — it does not say *which* key they are. The device-key ledger of §6.2 answers **who may hold a key**; it cannot answer **which key this is**, and those are independent questions. So a relay that never breaks ECIES, facing a perfectly correct ledger, could otherwise take an authentic wrap and re-present it in a different slot: Click A's group key delivered into Click B's distribution slot, or a pre-rotation key replayed as the current `group_key_version_id`. Both are damaging in the same direction, because the receiving device does not merely fail to decrypt — it succeeds, and then acts on what it believes it holds. An admin device convinced that Click A's key is Click B's current group key will re-wrap Click B's content keys under it ([section 6.3](#63-content-key-indirection)), and every holder of Click A's key thereby gains decrypt reach into Click B's roster: a cross-Click biometric leak produced by the relay alone, with no member misbehaving and no ledger entry out of place. A replayed pre-rotation wrap installs a key a departed member still holds, quietly undoing the revocation of [section 6.4](#64-biometric-group-key-rotation-on-member-removal). Binding the full `(world_id, click_id, group_key_version_id, recipient_account_id, recipient_device_id)` tuple into both the HKDF salt and the GCM AAD closes both cases: the derived key differs in every slot, so a relocated ciphertext does not authenticate, and the relay is left holding bytes it cannot move.

**Versioning.** `group-key-recipient-wrap-v1` is greenfield. myClick is pre-production, with no real users and no deployed group-key wrap to stay compatible with, so this is not a re-key of existing wraps and there is no dual-parser period: v1 is the first and only accepted form, and anything that is not a conforming `group-key-recipient-wrap-v1` is rejected rather than handed to a legacy path. This rides on the preproduction break [section 11.2.1](#1121-versioned-authenticated-envelope-and-aad) already requires of the §6.2 recipient-key gate — every active device re-proved through a fresh invite or device-link secret, an admin rotation of the biometric group key, every pre-gate wrap deleted before distribution resumes — so the first conforming wrap each device receives is minted under the new `group_key_version_id` that rotation creates. A later change to the sealed material, the context tuple, or the encoding requires a new record version, not a widened v1.

**Scope.** This record governs one artifact: the ECIES delivery wrap, the group key sealed to a member device's identity public key. It does not govern the symmetric wraps — the owner/admin durability seal of [section 6.8](#68-group-key-durability-owneradmin-escrow) and the per-person content-key wraps of [section 6.3](#63-content-key-indirection) — which are different artifacts under different outer keys. An earlier draft of this paragraph fenced those out as unpinned; they are now pinned in their own right, with their own context tuples and their own justifications, in [section 6.9](#69-exact-symmetric-key-wrap-records). Between the two sections the wrap family is closed: no key wrap in the biometric hierarchy is relocatable between Clicks, key versions, people, or devices. The one relocation surface that remains open is a layer lower — the fields sealed *under* a content key — and §6.9 states its bound explicitly.
...
Both arms of that per-audience wrap are pinned wire formats, not opaque blobs: `content-key-vault-wrap-v1` and `content-key-group-wrap-v1` in [section 6.9](#69-exact-symmetric-key-wrap-records), each context-bound to the person and the audience it belongs to. The binding is what makes "one key, many audiences" safe — an unbound wrap opens under the right outer key in the *wrong* slot, and one content key relocated across audiences is one child's biometric readable by a Click they were never opted into.
...
2. It is wrapped only to each remaining device key authenticated by §6.2's current Click device-key ledger, as the pinned `group-key-recipient-wrap-v1` of [section 6.2.3](#623-exact-biometric-group-key-recipient-wrap), bound to the new `group_key_version_id`.
...
The `group_key_vault_wrap` ciphertext this section stores is pinned as the closed `group-key-vault-wrap-v1` record of [section 6.9](#69-exact-symmetric-key-wrap-records), context-bound to the Click, the key version, and the owner account. The recovery of step 2 reconstructs that context from the slot it is recovering into and byte-compares it before opening; a blob served from a different Click's slot or a superseded version fails to authenticate rather than yielding a key the owner would then redistribute.

### 6.9 Exact symmetric key-wrap records

[Section 6.2.3](#623-exact-biometric-group-key-recipient-wrap) pinned the one wrap that travels to a member device sealed under an identity *public* key. It is not the only wrap that travels. Three more cross the relay sealed under a *symmetric* outer key — the group key escrowed under an owner's vault key ([section 6.8](#68-group-key-durability-owneradmin-escrow)), and each person's content key wrapped under the owner's vault key or under a Click GroupKeyVersion ([section 6.3](#63-content-key-indirection)) — and every one of them had the same gap for the same reason: the artifact was an opaque ciphertext whose meaning lived entirely in the row it happened to sit in. This section closes the family. The argument is §6.2.3's, and it is worth stating once in general form because it is what all four cases share: **an outer key answers who may open a wrap; it never answers which wrap this is.** Wherever those two questions have different answers, a relay can move an authentic ciphertext into another slot, and the holder does not merely fail — it opens, succeeds, and acts on what it believes it holds.

The vault key makes this sharp rather than theoretical. It is per **account** — not per Click, not per person ([section 3.1](#31-the-three-layers-precisely)) — so one vault key is the correct outer key for every group key that account escrows and every content key it owns. All of those ciphertexts open under the same key, so telling them apart is *entirely* the job of a context binding. Without one, the owner's own correct key is the attacker's instrument, and nothing anywhere in the system registers a fault.

**Why none of these binds a device.** §6.2.3 binds a `recipient_device_id` because an ECIES delivery wrap is sealed to one device's Enclave key and is meaningless anywhere else. None of the three records below binds a device, and that is a decision rather than an omission. The vault key is account-scoped and recovered from iCloud Keychain onto whatever device the owner is now holding ([section 10.2](#102-the-mechanics)); §6.8 exists *precisely* so that a new device can open a wrap minted by a dead one. A device binding would make these artifacts unopenable by the only device that ever needs them — a category error, not a stronger binding. The group key is by construction shared across every current member device, so binding one there would be wrong in the same way.

**The group-key vault escrow.** §6.8 seals the current group key under the owner's vault key, and the slot it fills is `(owner account, group key version)`. Using the canonical `ENCODE` of [section 11.2.1](#1121-versioned-authenticated-envelope-and-aad):

```text
group_key_vault_context =
  ENCODE("myclick.group-key-vault-wrap.v1",
         world_id, click_id, group_key_version_id, owner_account_id)

K_group_vault_wrap = HKDF-SHA256(
  IKM = vault_key,
  salt = SHA-256(group_key_vault_context),
  info = "myclick.group-key-vault-wrap-key.v1",
  L = 32)
```

AES-256-GCM then seals the exact 32-byte group key under `K_group_vault_wrap`, with one fresh 96-bit CSPRNG nonce and `group_key_vault_context` as AAD. The wire record is the closed `group-key-vault-wrap-v1` map in [section 11.2.2](#1122-canonical-v1-records). `click_id` is bound even though `group_key_version_id` already determines a Click, because that determination is a *relay-side join*: binding the Click explicitly lets the recovering device check against the Click it is installing into without trusting a mapping the relay supplied. `owner_account_id` is bound for a reason specific to this product rather than for symmetry — the vault key is escrowed in iCloud Keychain, so it follows the **Apple ID**, not the myClick account, and myClick expects shared Apple IDs (a school's `marketing@<school>` is the documented pattern). Where two accounts share one Keychain they share one vault key, and the account binding stops being redundant and becomes the only thing holding them apart.

*The relocation this defeats.* A relay serves the blob from `(this account, Click A's key version)` when the owner asked for `(this account, Click B's key version)`. It opens — it genuinely is the owner's own vault key, and nothing is forged. The owner now holds Click A's group key believing it is Click B's, and §6.8 step 2 tells them to re-wrap it under their fresh identity key and distribute it. Click B's devices install it, Click B's content keys are re-wrapped under it ([section 6.3](#63-content-key-indirection)), and every holder of Click A's group key — Click A's entire membership — gains decrypt reach into Click B's roster. A cross-Click biometric leak, produced by the relay alone, with no member misbehaving. The same move served from a superseded version of the *same* Click reinstates a key a departed member still holds, quietly undoing the revocation of [section 6.4](#64-biometric-group-key-rotation-on-member-removal).

**The content-key vault wrap.** §6.3 wraps each person's content key under the owner's vault key for owner-private access. The slot is `(owner account, person)`:

```text
content_key_vault_context =
  ENCODE("myclick.content-key-vault-wrap.v1",
         world_id, owner_account_id, person_id)

K_content_vault_wrap = HKDF-SHA256(
  IKM = vault_key,
  salt = SHA-256(content_key_vault_context),
  info = "myclick.content-key-vault-wrap-key.v1",
  L = 32)
```

AES-256-GCM seals the exact 32-byte content key under `K_content_vault_wrap`, one fresh 96-bit CSPRNG nonce, `content_key_vault_context` as AAD; the wire record is `content-key-vault-wrap-v1`. Neither `click_id` nor `group_key_version_id` appears, and that is correct rather than incomplete: the vault arm *is* the owner-private audience, it is not Click-scoped, and the data model holds no group key version for it. Binding either would invent a scope the artifact does not have and then require the owner to satisfy it.

*The relocation this defeats.* A guardian guards two children. The relay swaps their two vault wraps. Both open — one vault key, both genuinely the owner's — and the guardian now holds the first child's content key believing it is the second's. When they opt the second child into a school Click, they re-wrap what they believe is that child's content key under the school's group key. The school now holds the **first** child's content key, and the relay serves that child's embedding ciphertext to any member who asks. A child never opted into that Click has their face template become readable by it, with the guardian doing everything correctly at every step. This is the more reachable of the two vault relocations: it needs one guardian with two children and one wider Click, which is the ordinary family case, not an unusual one.

**The content-key group wrap.** §6.3 also wraps the same content key under each Click GroupKeyVersion the person is opted into. The slot is `(person, group key version)`, and the outer key is the group key rather than a vault key:

```text
content_key_group_context =
  ENCODE("myclick.content-key-group-wrap.v1",
         world_id, click_id, group_key_version_id, person_id)

K_content_group_wrap = HKDF-SHA256(
  IKM = group_key,
  salt = SHA-256(content_key_group_context),
  info = "myclick.content-key-group-wrap-key.v1",
  L = 32)
```

AES-256-GCM seals the exact 32-byte content key under `K_content_group_wrap`, one fresh 96-bit CSPRNG nonce, `content_key_group_context` as AAD; the wire record is `content-key-group-wrap-v1`. No `owner_account_id` is bound here, and that is a constraint rather than a saving. The opener is any current member of the Click, not the owner. A member does not reliably know which account holds a person's primary guardianship and should not need to — and guardianship legitimately changes without the content key changing, since adding a co-guardian ([section 6.7](#67-one-canonical-child-enrolment-per-click-co-guardianship-not-duplication)) touches governance, not keys. Binding a guardian account would let an unrelated governance event silently break every member's ability to open the wrap. Bind the audience and the subject; never bind a party the opener cannot independently see.

*The relocation this defeats.* Within one GroupKeyVersion every member opens every wrap under the same key, so the group key cannot distinguish one person's wrap from another's. A relay swaps the wrap rows for two people in that Click and swaps their embedding rows to match. Both open. The Click's roster now recognises the first child's face and calls them the second — and the per-person distribution consent read at Publish ([section 11.16](#1116-distribution-consent-freshness-at-publish)) is then read for the wrong child. A child whose guardian set distribution off is published under a sibling's consent. This is not a leak of the roster to outsiders; it is a break of the consent boundary *inside* it, which is the same promise seen from the other side.

**Reconstruct from the slot, never from the record.** As in §6.2.3, the context ids carried inside each record are the relay's routing copy — locators, never a trust root. Before opening any of the three, the holder reconstructs the context from the slot it is filling: the world and Click it is installing into, the key version that slot claims, the person whose roster entry it is reading, and its own account id. It byte-compares the record's copies against that reconstruction, and only then opens. A wrap presented in the wrong slot fails the GCM tag. An unknown version, a context mismatch, a duplicate wrap for the same context, or a failed tag rejects the material and retains nothing — it is never reconciled, repaired, or retried under a second interpretation.

**Rotation, re-wrap, and one implementation trap.** Because `group_key_version_id` is inside the context, the re-wrap that [section 6.4](#64-biometric-group-key-rotation-on-member-removal) already requires on every removal rotation is now also a re-*derivation*: the same content key sealed under a new version's context yields a different wrapping key and different bytes. An implementation that carries a prior version's wrap bytes forward into the new version's row is non-conforming, and will fail to open rather than fail quietly. The same follows for the rare deliberate content-key change of §6.3: each of the audiences that must be re-wrapped in one atomic transaction is now a distinct context, and a partial fan-out surfaces as a fail-closed unreadable Click ([section 7.3](#73-roster-decryption-lifecycle-r1)) rather than as a wrap that opens onto the wrong key.

**What the data model must carry.** Every stored wrap MUST represent its full context tuple explicitly, and the holder MUST reconstruct from that representation rather than from a join the relay performed. Deriving a wrap's Click by joining its key version, or its owner account by joining a guardianship table, is exactly the relay-supplied mapping the binding exists to distrust; a join result is a routing convenience and never an authenticated context field. Inferring which key a wrap carries from current membership, from the row it sits in, or by trial decryption against candidate keys is non-conforming — trial decryption in particular is the shape this construction is designed to make unnecessary, because a correctly bound wrap opens under exactly one context and no other.

**Versioning and the migration this forces.** All three records are greenfield. myClick is pre-production, with no real users and no deployed wrap to stay compatible with, so there is no dual-parser period: v1 is the first and only accepted form, and anything that is not conforming is rejected rather than handed to a legacy path. The migration implication is nonetheless real and worth stating plainly, because it is destructive. These constructions change the bytes of *every* stored symmetric wrap, so existing rows are not readable under the new derivation and MUST be deleted rather than migrated. There is deliberately no re-encryption path: converting an old row would mean opening it under the context-free construction and resealing it under an assumed context, which is precisely the blind operation the binding exists to prevent. Concretely, the escrow and content-key wrap tables are emptied, and the admin rotation that [section 11.2.1](#1121-versioned-authenticated-envelope-and-aad) already requires of the preproduction break re-mints every wrap under the new `group_key_version_id` in conforming form. A row that survives that wipe is treated as unreadable, not as legacy.

**Scope — the layer this does not close.** Below these three sits one more layer, and it is named here rather than left for a future reader to re-derive. The payload sealed under a person's content key — the embedding templates, and the display name and age band that [section 4](#4-enrolment-face-to-encrypted-embedding) seals under that same key — carries no context binding either. Those ciphertexts are identified only by their columns, and the fields are sealed with the same construction as one another, so at the AEAD layer a name ciphertext and a template ciphertext are mutually substitutable and two template slots are interchangeable. That is a real relocation surface of the same family. It is not pinned in this change because its correct binding is a genuinely different question rather than a copy of these three: `template_index` is reassigned by re-enrolment and `model_version` changes under an embedding-model upgrade, and [section 4.5](#45-re-enrolment-periodic-and-explicit-age-banded-no-silent-learning) (R-RE1–R-RE4) requires both to re-seal under the **same** content key. An AAD that fixed the index or the model version would break re-enrolment, so pinning it needs that interaction worked through first, and a wrong pin would be worse than a named gap. Until it is pinned the honest bound is this: the three records above stop a relay moving **keys** between audiences, and do not yet stop it shuffling **sealed fields** within one person's own key.
...
There is no honest single disk claim for all three custody paths. The normative claims are deliberately separate:

1. **Mode A, frame-accessible:** myClick never persists the clear source original; it exists only in bounded working memory.
...
3. **Mode B import:** the external provider/Photos library already owns the original. The attended importer reads it **in place** and never copies it into myClick scratch or pocket storage. myClick cannot make claims about the external owner's retention.

After a Mode A source enters durable myClick custody, only app-encrypted bytes may persist. The stronger "never touched disk" statement applies only to frame-accessible Mode A; "myClick keeps no imported-original copy" applies to Mode B.
...
- **Mode B:** an attended, foreground import reads a user-selected external source in place and emits only an obscured derivative; it is neither Flow A custody nor Flow B capture custody.
...
- **Mode B import is not Flow A custody.** It is attended and foreground-only. The importer opens the external source's user-authorised URL/Photos resource in place, decodes bounded frames or windows, obscures them, and writes an obscured derivative. It creates no myClick copy of the imported original, no pocket original, and no resumable background-original job. Background, lock, source revocation, or cancellation ends the batch and releases access; retry starts from the external source again. (Recognition behaviour is in [section 7.9](#79-mode-b).)

For frame-accessible Mode A, the source original lives and dies in memory before any myClick persistence. This statement does not describe Mode B's externally persisted source and does not describe an OS-owned Flow-B handoff.
...
- any unlink/absence-verification failure blocks every media surface and fails loud. Reconciliation is idempotent and resumes from the last durable state.
...
Once `encrypted_committed` is durable, the retained Mode A original is app ciphertext and can be crypto-shredded. Frame-accessible Mode A never creates the handoff. Mode B leaves its original with the external provider and makes no erasure claim about it.
...
- **Claims are path-specific.** Frame-accessible Mode A never persists a clear source; Mode B makes no myClick original copy; OS-owned Flow B briefly creates a Data-Protected clear handoff that is app-encrypted then platform-unlinked, with the flash-erasure residual stated in §8.5.
...
Three features hold **app-encrypted** blobs on disk: a Flow-B encrypted-import partial, the Mode-A **Light Table** pocket, and the received-obscured Stream cache. The clear OS handoff is not a lane and is never called encrypted scratch; it is an external custody obligation tracked by the §8.4 state machine until platform unlink is verified. Mode B external originals are also not a lane. Every actual store item has a per-item key and one retention lane.
...
**Minimisation.** The Mode-A pocket may retain an app-encrypted unobscured original; the Stream lane retains app-encrypted obscured media. Mode B retains no original copy. Each persistent lane has its own bound — root relay/local expiry for Stream and a bounded-with-a-nudge prompt for Mode-A pocket items. The store never silently destroys a pocket item the user has not decided on.

### 8.9 The Light Table: persistent review and the switch-off commit

The Light Table has two custody modes that share presentation but not persistence. **Mode A** is the pocket lane's persistent review surface. **Mode B** is an attended foreground import grid over externally-owned sources; it does not put imported originals in the pocket and does not survive background/lock as a resumable source batch.
...
**Mode B stays attended and source-external.** The one explicitly selected Click and roster version are frozen for the in-progress batch only; they are held in memory with the external provider handle, not written as a source pocket item or durable source bookmark. Immediately before an obscured derivative is exported or armed for Stream, the importer rechecks the operator's accepted capture grant and the selected Click's current roster version. If either changed, it re-renders from the still-open external source in the same foreground/unlocked session. If the source handle, permission, foreground/unlocked state, or roster cannot be re-established, the item is cancelled and any unfinished derivative/observations are shredded or zeroed; the app never resumes from a hidden source copy. Once the completed obscured derivative has crossed to Photos or another external destination, the usual forward-only limit applies.
...
**World/security domain.** `world_id` is defined in the canonical model at [section 2](#2-canonical-model), with its minting, distribution, unknown/stale handling, and one-world-per-deployment storage shape specified at [section 2.1](#21-the-world-in-practice). Restated here only as it bears on this envelope: it is a stable CSPRNG UUIDv4 assigned once to a deployment, public and immutable within it. Keys, rows, publications, and wraps MUST NOT cross worlds even when other UUIDs collide. In `AAD_base` below it is the outermost bound context field, and a recipient reconstructs it from its own pinned world rather than from the delivery record's copy of it.
...
The §6.2 recipient-key gate is part of the same preproduction break. Existing raw relay key rows MUST NOT be copied into a trusted device-key ledger. Each active device is re-proved through a fresh invite/device-link secret (or the Click is destructively re-created); then an admin rotates the biometric group key, re-wraps current content keys only to the authenticated ledger, creates a fresh Stream epoch, and deletes every pre-gate group/epoch recipient wrap before distribution resumes. That rotation is also the first distribution under the pinned §6.2.3 format: every biometric group-key wrap minted from this point is a conforming `group-key-recipient-wrap-v1`, and no pre-gate wrap is parsed, re-presented, or carried forward. The §6.9 symmetric wraps break at the same point and in the same way: every pre-gate `group_key_vault_wrap` row and every pre-gate content-key wrap — both the vault arm and each group arm — is **deleted, not converted**, because converting one would mean opening it under the old context-free construction and resealing it under an assumed context, which is the blind operation the binding exists to prevent. The rotation re-mints them as conforming `group-key-vault-wrap-v1`, `content-key-vault-wrap-v1`, and `content-key-group-wrap-v1` under the new `group_key_version_id`. A Click lacking a complete authenticated ledger remains unavailable, and any wrap row surviving the wipe is treated as unreadable rather than as legacy. This prevents migration from blessing a key that a previously malicious relay could have substituted.
...
group-key-recipient-wrap-v1 = {
  ...
  4: uuid,                     ; recipient_account_id
  5: uuid,                     ; recipient_device_id
  6: x963-p256-public,         ; ephemeral public key
  7: nonce96,
  8: bstr .size 32,            ; encrypted biometric group key
  9: tag128
}

group-key-vault-wrap-v1 = {
  0: 1,                        ; wrap_version
  1: uuid,                     ; world_id
  2: uuid,                     ; click_id
  3: uuid,                     ; group_key_version_id
  4: uuid,                     ; owner_account_id
  5: nonce96,
  6: bstr .size 32,            ; group key sealed under the vault key
  7: tag128
}

content-key-vault-wrap-v1 = {
  0: 1,                        ; wrap_version
  1: uuid,                     ; world_id
  2: uuid,                     ; owner_account_id
  3: uuid,                     ; person_id
  4: nonce96,
  5: bstr .size 32,            ; content key sealed under the vault key
  6: tag128
}

content-key-group-wrap-v1 = {
  0: 1,                        ; wrap_version
  1: uuid,                     ; world_id
  2: uuid,                     ; click_id
  3: uuid,                     ; group_key_version_id
  4: uuid,                     ; person_id
  5: nonce96,
  6: bstr .size 32,            ; content key sealed under the group key
  7: tag128
}
...
- **Symmetric key wraps.** `group-key-vault-wrap-v1`, `content-key-vault-wrap-v1`, and `content-key-group-wrap-v1` (§6.9) each seal exactly 32 bytes of key material under a wrapping key derived by HKDF-SHA256 from the outer key (the owner's vault key or the Click group key) with `SHA-256(context)` as salt and the record's own info label, and carry that same context as GCM AAD. Each is opened only after its complete context tuple has been reconstructed from the slot being filled — never from the record's own copies, and never from a relay-performed join such as key-version-to-Click or person-to-guardian — and byte-compared against them. None binds a device id. A duplicate wrap for the same context, an unknown version, a context mismatch, or a failed tag rejects the material and retains no key. Wrap bytes are never carried forward across a `group_key_version_id` change, and trial decryption against candidate outer keys is non-conforming.
- **Recipient wraps.** `group-key-recipient-wrap-v1` (§6.2.3) and `stream-epoch-recipient-wrap-v1` (§6.2.2) are opened only after their complete context tuple has been reconstructed from the slot being filled — never from the record's own copies — and byte-compared against them. Each carries exactly 32 bytes of ciphertext and one fresh ephemeral X9.63 public key used for one recipient and exactly one successful seal. A repeated ephemeral public key, a duplicate wrap for the same `(context, recipient_account_id, recipient_device_id)`, an invalid/off-curve/non-canonical point, an unknown version, or a failed tag rejects the grant and retains no key material. Neither record is accepted unless its recipient account, device, and public key match one authenticated current §6.2 ledger entry; a `group-key-recipient-wrap-v1` whose `group_key_version_id` disagrees with the ledger it subsequently opens is discarded rather than reconciled.
...
2. **Mode B local custody.** The attended importer reads the external source in place (§8.9), builds observations only in bounded memory, and may retain them only by atomically sealing the completed already-obscured derivative plus its publication sidecar into a device-key-encrypted outbound item. It MUST NOT retain the imported original, a source-equivalent transcode, durable source bookmark, plaintext observation DB, or resumable source job. Background, lock, source-access loss, cancellation, or a torn derivative/sidecar seal zeroes observations and cancels the unfinished item. A sealed outbound item is crypto-shredded after acknowledged publish or cancellation.
