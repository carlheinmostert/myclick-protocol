...
The three custody paths differ in **ingest**, not in **retention**. They are stated separately because how an original enters myClick differs materially — and because only one of them involves a clear file myClick did not write:
...
- **Ingest is path-specific; retention is not.** Frame-accessible Mode A and Mode B import both reach app-encrypted pocket custody without any clear source file ever existing; OS-owned Flow B briefly creates a Data-Protected clear handoff that is app-encrypted then platform-unlinked, with the flash-erasure residual stated in §8.5. All three then retain an app-encrypted unobscured original on identical pocket terms and crypto-shred it on publish or discard. The earlier claim that **Mode B makes no myClick original copy** is factually wrong and is withdrawn (corrected 2026-07-21); what survives it is the narrower and true claim that no *recoverable cleartext* original persists.
