...
**Manual narrowing (two-way Refine).** Within a softened depth-of-field render, the photographer MAY manually narrow **one specific person's** region from whole-body back to **face-only** using the depth-of-field Refine brush, which is bidirectional: **sharpen** narrows a person's extent toward face-only, **soften** returns it toward the whole-body melt. This is an **aesthetic Layer-2 control, floored at the face**: the manual override can only ever **shrink** the whole-body melt down to the face-focused obscure — it can never remove or shrink the **Layer-1 face obscure** itself, and the extent has no representable value below face-only. For an **unrecognised (stranger)** subject, sharpen brings the **body** back into the composition while the **face stays obscured**: a stranger remains structurally ineligible to be kept sharp, its body is traced into the sharp region but its head is knocked out of that region and obscured by the unchanged stranger-head machinery ([section 7.12](#712-depth-of-field-the-scene-blur-may-satisfy-the-anonymity-floor)), fail-closed. The narrowing carries **no new decision and nothing on the wire**, and the **consent-distribution re-blur paths are unaffected** — a consent-excluded recognised subject inside a softened destination render keeps the whole-body melt ([section 11.1](#111-two-obscured-versions-on-the-photographers-device)). The manual override is not a reveal; revealing a person is the separate, consent-gated identify flow. Recorded in [ADR-0039](https://github.com/carlheinmostert/myClick/blob/main/docs/adr/0039-refine-two-way-brush-per-person-face-only-obscure-override.md).
...
3. **One surface, one lease.** Each file is owned by one active playback lease. It is removed and verified absent when that surface exits, when playback fails or is cancelled, when the app backgrounds, when the device locks, and when the publication is retracted, withdrawal-denied, expired, or found version-stale. V1 has no successor or replacement lease.
4. **Launch sweep before availability.** On every launch, before the Stream or any playback surface becomes available, the app enumerates the dedicated directory and removes every file as a crash orphan. There is no resume of a prior clear playback file.
...
The authenticated publish RPC atomically admits one payload envelope, one mandatory observation-sidecar envelope, one `K_blob` wrap, and one `relay-row-v1`, or admits nothing. Objects use neutral internal storage references and `application/octet-stream`; recipients never receive a raw bucket path or pre-signed URL. The complete relay-visible inventory is the row plus epoch/cohort ledgers, RPC/auth logs, object ciphertext/length/timing, fetch tickets while live, and APNs routing metadata enumerated in §11.18. There is no predecessor, successor, or lineage field.
...
After both redemptions, the client still performs every cryptographic/context check in §11.2.1 before display. There is **no bypass RPC**: v1 defines no `fetch_for_reblur`, no entitlement-exempt content read, no direct-storage path, and no delivery path around the standing withdrawal gate.
