...
#### 6.2.1 The photographer capability is separate from admission, and withdrawable by its subject
...
The photographer capability is acquired through a **two-party handshake**: the member **requests** it (the app-domain `request_capture`), and a Click **admin approves or declines** it (the app-domain `approve_capture` / `decline_capture`). The request is a **durable, declinable state** — it persists until an admin acts on it **or the member who filed it withdraws it** (below), exactly as the guardianship request of [section 6.7.1](#671-the-request-direction-when-the-second-guardian-initiates) does — not a fire-and-forget call. This mirrors the existing two-sided shape already used for guardianship and for admission itself: one party initiates, a second party with authority consents, and neither side can complete the grant alone **while the two sides are different principals**.

**The admin carve-out — when the requester and the approver are one principal.** The two-party rule exists to protect a *member* **from** an *admin*: it is what stops an admin dropping someone else into a biometric-processing role. When both roles are held by the same account — a Click admin acquiring the capability for their **own** membership — there is no second party to protect, and no consent is manufactured by making that admin file a request with themselves and then approve it. Splitting the act would also strand the admin in a `requested` state on any partial failure, visible to themselves in their own approval queue. An admin therefore acquires the capability for their own membership in **one atomic act** (the app-domain `self_grant_capture`), with no intermediate requested state. This is a carve-out on the *procedure*, never on the *record*: a self-grant is stamped with its own distinct request state (`self_granted`, never `approved`), so the durable record still says **how** the capability arose, and an auditor — a school reconstructing who was authorised to photograph, and on whose say-so — can always tell an admin's self-grant from a member's request that an admin approved. The carve-out is strictly self-scoped: it authorises an admin to grant only to their own membership, never to another account's, which still requires that member's own request.
...
**Withdrawal — four named acts, on two different objects.** Two of them end a **grant already in force**: it can be taken away by the **admin** who gates it (the app-domain `revoke_capture`: admin-scoped, targets one member, and turns an existing grant off), and it can be given up by **its own subject** (the app-domain `relinquish_capture`). Two others end an **outstanding request**, before any grant exists: an **admin** can turn it down (the app-domain `decline_capture`), and the **member who filed it** can withdraw it (the app-domain `cancel_capture_request`). The two objects are not interchangeable — `decline_capture` and `cancel_capture_request` never touch a grant already in force, and `revoke_capture` and `relinquish_capture` are never a substitute for deciding an outstanding request. None of the four removes the member from the Click — departure is [section 6.4](#64-biometric-group-key-rotation-on-member-removal)'s separate act with its own rotation consequences.

**Relinquishment is self-scoped and unilateral.** A member holding the photographer capability MAY withdraw it from their own membership at any time, without an admin's approval and without an admin being able to block it. This **extends** the two-party handshake above rather than contradicting it: the handshake governs how the capability is *acquired*, and it is asymmetric on purpose — it takes two parties to start processing other people's children's faces, and one to stop. It is compelled by the same rationale that grounds the handshake. If the capability must "originate from the data-subject's own affirmative act", then it is a **consent**, and a consent that its subject cannot withdraw is not a consent at all (POPIA s.11(2)(b); GDPR Art. 7(3), which requires withdrawal to be as easy as giving it). An implementation that offers a member no way out of the photographer role is therefore non-conforming, not merely incomplete. Relinquishment returns the membership's request state to the neutral `none` rather than `declined` — the member was not turned down, and must be able to ask again later without an admin first clearing a state they did not cause.

**Relinquishment is cryptographically inert.** Like acquisition, it is a **membership-attribute change only** — it flips `can_capture` back off for one member. It distributes **no key material, mints no GroupKeyVersion, and MUST NOT trigger a group-key rotation.** The symmetry a reader might expect with [section 6.4](#64-biometric-group-key-rotation-on-member-removal) does not hold, and the reason is that the two acts remove different things. Removal takes away **membership**, and with it the entitlement to decrypt the roster at all — so the key that member holds must stop being the current key. Relinquishment takes away only the right to **capture under** the Click; the relinquishing account remains a member, remains entitled to the roster it was admitted to, and legitimately keeps the group key it already holds. Rotating on relinquishment would re-wrap the entire Click for no change in who may decrypt what, and — in a large school Click — would make a member's own withdrawal expensive enough to discourage offering it. Rotation remains reserved for a member **leaving or being removed**. The same holds for the admin-side `revoke_capture`.

**A request may be withdrawn by the account that filed it.** A member holding an outstanding request MAY withdraw it at any time before an admin has acted on it, without an admin's involvement and without an admin being able to block it. Like relinquishment it is strictly **self-scoped** — the subject is the calling account, never a parameter — so it can only ever reach the requester's own membership, and it is not a means of clearing anyone else's entry from an approval queue. It rests on the same rationale as relinquishment: if the capability must "originate from the data-subject's own affirmative act", then the subject must be able to take that act back while it is still merely an act of asking. A request that could be filed but never retracted would leave a member's own asking standing on the record indefinitely at an admin's discretion, which is the same defect — a consent the subject cannot get out of — one step earlier in the cycle.

Withdrawal returns the membership's request state to the neutral `none`, and **MUST NOT** stamp it `declined`. `declined` is an *admin's* decision; recording it here would attribute the act to an admin who never made one, and would corrupt exactly the audit trail a school depends on to reconstruct who decided what and on whose say-so — the same distinction the `self_granted` stamp exists to preserve. `none` is also the state the member must be in to ask again later. The act clears **only** the request state: a grant already in force is left untouched, so a membership that holds both a live grant and a later outstanding request keeps the grant (surrendering that is `relinquish_capture`'s act, and conflating the two would silently strip a capability the member still holds and still consents to).

Like acquisition and relinquishment, withdrawing a request is **cryptographically inert** — it distributes no key material, mints no GroupKeyVersion, and MUST NOT trigger a group-key rotation. It **notifies nobody**: no admin had yet acted, so there is no counterparty owed a message, and the request simply ceases to appear in the approval queue it was waiting in. An implementation that emits a notification, retracts a delivery record, or rotates on this act is non-conforming.
