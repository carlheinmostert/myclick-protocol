...
**Scope.** This record governs the ECIES delivery wrap: the group key sealed to a member device's identity public key. The owner/admin durability seal of [section 6.8](#68-group-key-durability-owneradmin-escrow) — the group key sealed symmetrically under the owner's own vault key and stored server-side as `group_key_vault_wrap` — is a different artifact under a different key, and is not pinned by this record.
